...
unsigned char PermutationNumber;
...
int main() {
	if (scanf("%hhu", &PermutationNumber)) {}
...
void Backtracking(unsigned char StackPeak) {
	for (unsigned char Index = 1; Index <= PermutationNumber; Index++) {
...
unsigned char isSolution(unsigned char StackPeak) {
	return StackPeak == PermutationNumber;
}
