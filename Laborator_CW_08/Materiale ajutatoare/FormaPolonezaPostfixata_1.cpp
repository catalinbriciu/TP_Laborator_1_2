...
{
	E_PRIO_1 = 0x00, //lowest - +/-
	E_PRIO_2 = 0x01, // medium */^
	E_PRIO_3 = 0x02, // (high prio) - ()
	E_MAX_PRIO = 0x03,
	E_INVALID_PRIO
};
typedef enum E_TYPE
...
typedef struct S_MAP //look-up table
{
...
void Compute_PolishForm()
{
	...
	do
	{
		...
		else
		{
			...
			{
				Push(MyStack, &StackPointer, ElementToCheck);
			}
			else
			{
				...
				else
				{
					...
					do
					{
					  ...
					  else
					  {
						  ...
						  elementInfoTemp = GetOperatorElementInfo(elementInfoTemp.identifier);
